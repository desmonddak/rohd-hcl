...
// The algorithm needs to extend with zeros and handle.
  ...
  factory FloatingPointValue.ofDouble(double inDouble,
    ...
    if (roundingMode == FloatingPointRoundingMode.roundNearestEven) {
      ...
      // print('mant64=${mantissa64.bitString}');
      final roundPos = 51 - (mantissaWidth + 2) + 1;
      final round = mantissa64[roundPos];
      final guard = mantissa64[roundPos + 1];
      final lsb = mantissa64[roundPos + 2];
      // print('lsb   =${' ' * (51 - (roundPos + 2))}L=${lsb.bitString}');
      // print('guard =${' ' * (51 - (roundPos + 1))}G=${guard.bitString}');
      // print('round =${' ' * (51 - (roundPos + 0))}R=${round.bitString}');
      // print('sticky=${' ' * (51 - (roundPos - 1))}Sticky=${sticky.bitString}');
      // print('mant0=${mantissa[0].bitString}');
